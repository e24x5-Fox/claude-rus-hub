...
### Как называются релизы

Репозиторий общий на все игры, поэтому «последний релиз» здесь не один на всех:
...
на сайте сама находит самый свежий релиз с этим префиксом, подставляет прямую
ссылку на файл, версию и размер. После выпуска новой версии на сайте править
нечего — достаточно назвать тег по образцу и приложить файл установщика с тем
же именем (`release.asset`). Если GitHub API недоступен, кнопка ведёт на общий
список релизов.
